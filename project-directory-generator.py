...
# dont't forget to make the project dir as the root folder.
...
def create_app():
    ...
    with app.app_context():
        ...
        from .home.routes import home as home_blueprint
...
class MakeFlaskProjectDirectories:
    ...
    def create_env_files(self):
        self.write(os.path.join(self.project_dir, '.env'),
                   dotenvfile_template)
        self.write(os.path.join(self.project_dir, '.flaskenv'),
                   dotflaskenv_template)
    ...
    def create_template_files(self):
        self.write(os.path.join(self.app_dir, 'templates', 'base.html'),
                   html_template)
        if self.modular_app_flag:
            self.write(os.path.join(self.app_dir, 'templates', 'auth', 'login.html'), html_template)
            self.write(os.path.join(self.app_dir, 'templates', 'auth', 'register.html'), html_template)
            self.write(os.path.join(self.app_dir, 'templates', 'home', 'index.html'), html_template)
        else:
            if self.template_names is not None:
                for file_name in self.template_names:
                    html_file = str(file_name) + str('.html')
                    self.write(os.path.join(self.app_dir, 'templates', html_file),
                               html_template)
    ...
    def create_init_file(self):
        ...
        self.create_file(os.path.join(self.project_dir, '__init__.py'))
    ...
    def create_routes_files(self):
        if self.modular_app_flag:
            ...
            with open(os.path.join(self.app_dir, 'home', 'routes.py'), 'a') as f:
